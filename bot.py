...
async def on_message(message):
	...
	if msg in answer_words:
		await message.channel.send('Я отправил вам команды в специальный канал ``#🧭навигация`` секретное сообщение! Тс-с-с.')
...
# Карточка
@client.command(aliases = ['ранг', 'я', 'карта', 'карточка', 'card'])
async def card_user(ctx):
	await ctx.channel.purge(limit = 1)

	img = Image.new('RGBA', (350, 170), '#232529')
	...
	headline = ImageFont.truetype('bahnschrift.ttf', size = 20)
	undertext = ImageFont.truetype('bahnschrift.ttf', size = 12)

	idraw.text((145, 15), f'Имя: {name}#{tag}', font = headline)
	idraw.text((145, 37), f'Айди: {ctx.author.id}', font = undertext)
	idraw.text((145, 60), f'Статус: {ctx.author.status}', font = undertext)
	idraw.text((145, 75), f'Активность: {ctx.author.activity}', font = undertext)
	idraw.text((293, 153), f'Deceit RU', font = undertext)
